const resume_id=new URLSearchParams(location.search).get("resume_id");(!resume_id||!localStorage.getItem(resume_id))&&location.replace("/resume-generator/");const data=JSON.parse(raw);r_name.textContent=data.candidateName||"",r_signature.textContent=data.candidateName||"",r_address.textContent=data.address||"",r_mobile.textContent=data.mobile||"",r_email.textContent=data.email||"",r_dob.textContent=data.dob||"",r_father.textContent=data.fatherName||"",r_mother.textContent=data.motherName||"",r_gender.textContent=data.gender||"",r_nationality.textContent=data.nationality||"",r_marital.textContent=data.maritalStatus||"",r_language.textContent=data.language||"",r_hobbies.textContent=data.hobbies||"",r_objective.textContent=data.objective||"",r_workExperience.textContent=data.workExperience||"",resumeType.textContent=data.type||"Resume",r_photo.src=data.photo||"";const academicTable=document.getElementById("academicData");data.academic.forEach(e=>{const t=document.createElement("tr");t.innerHTML=`
<td>${e.exam}</td>
<td>${e.board}</td>
<td>${e.year}</td>
<td>${e.marks}</td>
